...
class Heist(commands.Cog):
    ...
    async def heist(self, ctx, user: discord.User):
        ...
        if leader_balance < start_cost: 
            return await self.utils.notify(ctx, "Can't Start", f"You need at least {start_cost:,} Eurodollar's to start a heist", "Heist")

        target_balance = self.transactions.get_balance(target_id)
        if target_balance < target_limit: 
            return await self.utils.notify(ctx, "Can't Start", f"{user} has less than {target_limit:,} Eurodollar's", "Heist")
        ...
        screen = discord.Embed(title="Heist Started", description=f"{ctx.author.mention} has started a heist targeting {user.mention} /joinheist to join in", color=self.utils.DEFAULT_COLOR)
        screen.set_thumbnail(url="https://images-ext-1.discordapp.net/external/pq6uWSdWFXqOyDcktN0qSBzSkL3Txrk1gEGyoLmeXXE/%3Fsize%3D1024/https/cdn.discordapp.com/avatars/1021246946649849867/118da25b10cf765d4472c4664df8dd63.png?width=468&height=468")
        ...
        if player_left == 0:
            await self.endheist(ctx, target_id)
        ...
        if(success[0] == False):
            await self.endheist(ctx, target_id)
            return await self.utils.notify(ctx, "Heist FAILED", f"The heist has failed, all startup and contribution costs have been lost", "Heist")
    ...
    async def endheist(self, ctx, target_id):
        self.players.clear()
        self.alive.clear()
        self.dead.clear()
